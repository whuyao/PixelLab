...
- 前端已经重构为 `首页 / 市场 / 企业竞争 / 游客观察 / 生活与住房 / 财政与政府 / 小镇微博 / 晨报与案件 / 日志与观察 / 智能体教学` 十个子页面视图，避免把所有模块堆在一个页面里
...
- `游客观察` 已经拆成独立页面，集中看游客总池、活跃游客、偏好热词、消费、投资、赌博、看房与游客微博热度，不再把游客只塞在市场页的角落里
- 游客会优先使用自然中文名字；旧存档里带数字后缀的游客名会自动清洗，不再出现 `许栀12` 这类名字
...
![PixelLab 游客观察页](docs/images/tourism-observatory.png)
...
- `游客观察`：看游客总览、游客走势、重点游客、游客投资和游客偏好
- `生活与住房`：看生活压力、住房品质、房产结构、消费热度和住房市场
...
- 当前系统设计与实现：
  ...
  - [long_run_system_retrospective_1685d.md](docs/long_run_system_retrospective_1685d.md)：从第 1 天到第 1685 天的长周期复盘，集中讨论系统构建、人物发展、经济耦合、政府制度、地理空间与整体涌现
...
- 会参考每日匿名民意投票，在 `收入 / 秩序 / 发展` 之间做更细的平衡
...
- 每天固定更新一次匿名投票
- 玩家页面读取当天结果时不会实时重抽
- 投票不再走硬阈值，而是先算连续得分，再按 `支持 / 中立 / 反对` 的概率分布稳定抽样
- 大政府模式会根据当天民意偏向，微调税负、监管、利率和公共服务节奏
...
- 地图中有固定资产 `后巷地下赌场`
- 玩家走到门口后可直接触发赌局
- 智能体和游客会在资金紧张、资金充裕、情绪高涨、夜晚气氛升温时低频自主赌博
- `repeat / vip` 游客更容易被牌桌和围观气氛带动，赌场越热，游客越容易被卷进去试一把
...
- 页面会集中显示：
  ...
  - 重点游客跟踪
  - 游客名录
- 游客命名现在优先分配自然中文名，旧状态中带数字后缀的游客名会在读取时自动清洗并重新分配唯一中文名
- 当前系统采用分层游客机制：
  - `总游客池 = 10`
  - `活跃游客上限 = 7`
  - 只有活跃游客参与地图高频移动和高频行为
  - 非活跃游客仍保留在状态中，继续承载轻量消费、投资、微博和记忆信息
- 这样做的目的，是让游客层更热闹，但不把地图、日志和 token 成本全部压爆
...
- `PX100`：全市场综合指数
- `PX-Tour`：游客、住宿、零售与娱乐指数
- `PX-Life`：生活消费、住房与生活成本指数
...
`GEO / AGR / SIG / HSP / RET / EST / FIN / ENT` 会根据市场阶段、游客经济、企业舆情、地产压力、银行状态和赌场热度轮流成为主线。

典型规律：

- 牛市里 `GEO / SIG / FIN` 更容易领跑
- 游客回暖时 `HSP / RET / ENT` 更容易升温
- 房住压力抬高时 `EST / PX-Life` 更容易被点名
- 风险市里 `AGR` 更容易抗跌，`SIG / ENT` 更容易承压

### 指数与板块热度

- `PX100` 主要观察全市场风险偏好和综合方向
- `PX-Tour` 更敏感于游客、住宿、零售和娱乐
- `PX-Life` 更敏感于消费、住房、地产和生活成本
- 市场页的 `板块热度` 会综合涨跌、换手、企业客流和舆情热度排序
- 板块热度现在不再封顶 `100`，避免所有热门板块都挤成同一档位
...
- 总游客池上限为 `10`
- 同时高频活跃游客上限为 `7`
- 超出活跃阈值的游客会保留在状态里，但只做轻量更新，不会每轮都上地图或参与高频消费、聊天、投资与信号传播
