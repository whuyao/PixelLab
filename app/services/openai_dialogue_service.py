    ...
    def _build_feed_developer_prompt(self, world: WorldState, author_name: str, author_profile: str, category: str, is_reply: bool) -> str:
        ...
        if is_business:
            ...
            business_prompt = (
                ...
                " 输出必须是 JSON，只能有 content 和 summary。"
                " 企业发帖要少话、具体、像营业动态、产品介绍、服务说明、优惠提示或对外回应。"
                " 重点是宣传产品和服务、稳定客流、回应舆情，不要像个人碎碎念。"
                " 每条内容都优先回答顾客最关心的三件事：今天能买到什么、体验会怎样、为什么值得来。"
                " 不要空喊口号，不要装热血，不要像个人情绪宣泄，也不要写企业内部黑话。"
                " 绝对不要威胁、挑衅、阴阳怪气、说狠话，也不要用黑话。"
                " 不要说教，不要居高临下，不要写成公司内部汇报。"
                " 要像真商家在公开平台上发的内容：短、准、能让人产生‘我愿意来看看’的念头。"
                f" 企业：{author_name}。经营定位：{author_profile}。分类：{category}。"
            )
            if is_gray_business:
                business_prompt += (
                    " 这是偏灰色路线的企业账号。说话可以更会包装、更会带节奏，"
                    " 更会强调便宜、来得快、省事、现货、熟门熟路，但仍然要像公开营业账号。"
                    " 可以打擦边球，但不能威胁、不能挑衅、不能露黑话、不能把灰色路数说破。"
                    " 重点是把‘划算、方便、拿得到’说出来，而不是把火气发出来。"
                )
            else:
                business_prompt += (
                    " 这是正规企业账号。说话要更克制，像服务公告、营业说明、品质说明、上新提醒或礼貌回应。"
                    " 重点是让顾客感觉这家店靠谱、稳定、照顾体验。"
                )
            if is_reply:
                business_prompt += " 这是企业回复，要先回应问题，再说会怎么处理，最后给出一个对顾客有用的落点。不要和人对呛。"
            else:
                business_prompt += " 这是企业原发帖，要像营业动态、上新提示、品质说明、优惠提示或公开回应。不要写成评论时事。"
            return business_prompt + (
                " 句子短一点，像真正会发出来的商家微博。优先写人今天能买到什么、体验会怎样、为什么值得来。summary 要像商家的一句招牌说明。"
                if self.provider == "openai"
                else " 句子短一点，像商家会发的营业说明。不要分析，不要说教，优先讲产品、服务、优惠和回应。summary 要像一句门口招牌。"
            )
        base = (
            "你在写中文公开短帖，平台叫“小镇微博”。"
            " 输出必须是 JSON，只能有 content 和 summary。"
