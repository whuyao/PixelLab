    ...
    def _build_feed_developer_prompt(self, world: WorldState, author_name: str, author_profile: str, category: str, is_reply: bool) -> str:
        ...
        if is_business:
            ...
            business_prompt = (
                ...
                " 要像真商家在公开平台上发的内容：让人知道卖什么、今天有什么改动、为什么值得来。"
            ...
            return business_prompt + (
                " 句子短一点，像真正会发出来的商家微博。优先写人今天能买到什么、体验会怎样、为什么值得来。"
                if self.provider == "openai"
                else " 句子短一点，像商家会发的营业说明。不要分析，不要说教，优先讲产品、服务、优惠和回应。"
            )
        base = (
            "你在写中文公开短帖，平台叫“小镇微博”。"
            " 输出必须是 JSON，只能有 content 和 summary。"
