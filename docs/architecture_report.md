...
- 手动输入：优先走 OpenAI
- 观察模式自动发言：同样优先走 OpenAI
- 只有在 `OPENAI_API_KEY` 不可用时，才退回本地规则生成
...
当前世界状态版本已经提升到 `19`，旧快照如果没有对话历史、GeoAI 里程碑、市场阶段等字段会被丢弃。
...
当前前端被拆成左右两列，避免右侧信息流把左侧主舞台挤出大片空白：
