...
- `小镇微博`
  ...
  - 热帖传播链、围观人数和高热作者
  - 自动发帖与自动回帖支持 LLM 精修
  - 支持 `只看游客投资 / 只看政府回应 / 只看赌场传闻 / 只看企业发帖 / 只看商品质量 / 只看员工待遇`
  - 支持按情绪筛选
  - 支持 `阅读锁定`
...
### 7.1.1a 企业竞争作为经营与舆论耦合层

企业竞争现在已经是独立页面，而不是市场页里的一个小模块。当前小镇里接入了：
...
1. 经营链
- 居民和游客会按价格、品质、口碑和策略分流
- 企业会按时段和日级更新收入、成本和净流

2. 微博链
- 玩家、智能体和游客会讨论商品质量、服务态度、员工待遇和灰色路数
- 企业会自己发帖、回应、解释、打折或反驳
- 企业账号已经分成两类公开风格：
  - 正规企业：更克制，更像营业说明、服务提示、上新说明和品质说明
  - 灰色企业：更会包装，更会强调便宜、现货和来得值，但仍然不能直接威胁用户
- `小镇微博` 已支持筛选：
  - 企业发帖
  - 商品质量
  - 员工待遇
...
3. 灰市与制度链
- 后街商行会把压价抢客、灰色引流带进灰案和监管
- 企业营收已经产生营业税，并进入政府财政收入
- 舆情过热、价格过激或灰色竞争会提高被监管观察的概率
- 如果企业公开话术出现明显威胁、挑衅或“爱买不买 / 不买拉倒 / 后果自负”这类表达，系统会立即惩罚：
  - 企业口碑下降
  - 帖子可信度下降
  - 负面热度上升
  - 日利润和累计利润受损
  - 亏损连续天数抬高，更容易进入收缩、停业或并购链
...
因此，企业竞争不只是“再加几家店”，而是一个把经营、舆论、灰市、税收和地图空间绑在一起的新系统层。

### 7.1.1b 游客观察作为外来需求层观察台

游客系统原先只以零散卡片出现在市场页里，但随着游客开始承担：
...
这些角色已经不适合继续被当成“附属信息”。因此前端新增了 `游客观察` 独立页面，把游客作为一个完整的外来需求层来观察。

当前页面主要回答 5 个问题：

1. 现在小镇里来了多少游客  
2. 真正活跃并在地图上跑动的是哪些游客  
3. 游客在消费、投资、赌博和看房上分别怎样行动  
4. 哪些游客正在显著影响微博、市场和房产  
5. 游客偏好正在往什么方向集中  
...
也就是说，系统允许有 10 个游客持续存在于状态里，但只让其中 7 个承担地图高频移动、高频聚集和高频行为。其余游客仍保留在：
...
游客命名也已经从早期的“名字 + 数字后缀”逻辑改成自然中文名优先分配。当前状态归一化会自动清洗旧存档中的数字后缀，并重新分配唯一中文名，避免出现 `许栀12` 这类不自然的显示。
...
- 投票主体包括智能体与游客
- 每天只更新一次，不会在同一天内反复重算
- 投票不是硬阈值分类，而是先为每个个体计算连续得分，再抽样落到 `支持 / 中立 / 反对`
- 大政府模式会根据当天的支持、中立、反对结构，调整税负、监管、利率与公共服务的微调方向
...
生活消费、住房质量和房产结构现在被单独整理成 `生活与住房` 页面，不再只是左侧功能残块。
...
当前市场已经从早期的 `3` 只教学股票，扩成了 `8` 只可交易股票：
...
因此市场已经不再只是 `GEO / AGR / SIG` 三个教学 ticker 轮动，而是整个小镇经济结构的镜像。
...
计算得到的动态排序值。最近一轮调整后，板块热度已经取消 `100` 的上限，避免所有热门板块都堆在同一档位，失去区分度。
- 托底保护
- 随机冲击
...
也就是说，游客不是每一轮都重新随机挑一个点，而是会在空间上形成更平滑、更分散的移动轨迹；超过活跃阈值的游客则进入轻量更新模式，只保留状态，不参与高频地图与行为循环。
